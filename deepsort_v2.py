...
# 初始化DeepSORT
tracker = DeepSort(max_age=30, n_init=3)  # max_age: 目标消失后保留的帧数
...
class_id = 6
frame_count = 0  # 帧计数器
confidence_history = {}  # 存储每个目标的置信度历史
priority_target = None  # 优先跟踪的目标
reset_threshold = 30  # DeepSORT重置阈值
...
def draw(image, boxes, scores, classes, ratio, padding, tracks):
    ...
    for track in tracks:
        ...
        # 如果是优先目标，绘制红色框并发送数据
        if track_id == priority_target:
            cv2.rectangle(image, (int(ltrb[0]), int(ltrb[1])), (int(ltrb[2]), int(ltrb[3])), (0, 0, 255), 2)
            sender(center_x, center_y, class_id)
            print("优先目标:", center_x, center_y, class_id)
        else:
            # 非优先目标绘制蓝色框
...
def update_confidence_history(tracks, scores):
    global confidence_history

    for track in tracks:
        track_id = track.track_id
        if track_id not in confidence_history:
            confidence_history[track_id] = []
        confidence_history[track_id].append(scores[track.det_class])

        # 只保留最近10帧的置信度
        if len(confidence_history[track_id]) > 10:
            confidence_history[track_id].pop(0)


def select_priority_target():
    global confidence_history, priority_target

    if not confidence_history:
        return None

    # 计算每个目标的平均置信度
    avg_confidence = {track_id: np.mean(confidences) for track_id, confidences in confidence_history.items()}

    # 选择平均置信度最高的目标
    priority_target = max(avg_confidence, key=avg_confidence.get)
    return priority_target


def reset_tracker():
    global tracker, confidence_history, priority_target, frame_count
    tracker = DeepSort(max_age=10, n_init=3)  # 重置DeepSORT
    confidence_history = {}  # 清空置信度历史
    priority_target = None  # 清空优先目标
...
def myFunc(rknn_lite, IMG):
    global frame_count, priority_target
    ...
    if wp.digitalRead(GPIO_PIN) == wp.HIGH:
        if boxes is not None:
            ...
            # 更新置信度历史
            update_confidence_history(tracks, scores)

            # 每10帧选择一次优先目标
            if frame_count % 10 == 0:
                priority_target = select_priority_target()
                print("选择优先目标:", priority_target)

            # 如果优先目标消失超过阈值，重置跟踪器
            if priority_target and not any(track.track_id == priority_target for track in tracks):
                reset_tracker()
